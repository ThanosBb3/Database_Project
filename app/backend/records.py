...
def serv_rec():
    ...
    if request.method == 'POST':
        ...
        if not datefrom and not dateto and not costfrom and not costto and not service_type:
            flash('At least 1 field must be filled', category='error')
            status=400
        else:
            if not datefrom:
                datefrom = "1900-00-00"
            ...
            return render_template("serv_rec.html", results=results)

    return render_template("serv_rec.html", results=None)        